...
class LinearSystem(object):
    ...
    def multiply_coefficient_and_row(self, coefficient, row):
        n = self[row].normalized
        k = self[row].normalized

        new_normal_vector = n.scalar(coeffcient)
    ...
    def clear_coefficients_below(sel, row, col):
        num_equations = len(self)
    ...
    def clear_coeffcients_above(self, row, col):
    ...
    def compute_solution(self):
        try:
            return self.do_gaussian_alimination_and_extract_solution()

        except Exception as e:
            if (str(e) == self.NO_SOLUTIONS_MSG or
                str(e) == self.INFO_SOLUTIONS_MSG):
                return str(e)
            else:
                raise e
